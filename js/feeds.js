...
function setGrid() {
  if (STATE.layout === 'list') feedEl.className = 'grid gap-3';
...
function cardHtml(item) {
  ...
  return `<div class="card-wrap hover:scale-[1.01] transition"><article class="card p-3">
    ${img}
    ...
    <div class="mt-3"><a href="${item.url}" target="_blank" rel="noopener" class="btn-subtle">Open</a></div>
  </article></div>`;
}
function rowHtml(item) {
  const t = item.title || 'Untitled';
  const time = item.published_at ? new Date(item.published_at).toLocaleString() : '—';
  return `<a href="${item.url}" target="_blank" rel="noopener" class="row">
...
function render() {
  ...
  if (STATE.layout === 'list') items.forEach(it => feedEl.insertAdjacentHTML('beforeend', rowHtml(it)));
  else items.forEach(it => feedEl.insertAdjacentHTML('beforeend', cardHtml(it)));
}
function skeletons(n=9) { feedEl.innerHTML=''; for (let i=0;i<n;i++){ const d=document.createElement('div'); d.className='skeleton-card'; feedEl.appendChild(d); } }
...
async function loadSources() {
  try {
    ...
    const data = await res.json();
    const byTopic = {};
    for (const s of (data.sources || [])) { const t=s.topic||'Other'; (byTopic[t] ||= []).push(s); }
    ...
    Object.keys(byTopic).sort().forEach(topic => {
      ...
      const ul = document.createElement('ul'); ul.className = 'space-y-1 text-sm text-slate-300';
      byTopic[topic].forEach(s => { const li=document.createElement('li'); li.innerHTML = `<span class="badge">${s.type||'rss'}</span> <span class="ml-1">${s.name}</span>`; ul.appendChild(li); });
